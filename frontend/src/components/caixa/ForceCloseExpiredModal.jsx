...
export default function ForceCloseExpiredModal({ open, pendingCaixa, onClose, onClosed }) {
  ...
  useEffect(() => {
    if (open && pendingCaixa?._id) {
      setCountedAmount('');
      setObservacao('');
      caixaService
        .resumo(pendingCaixa._id)
        .then((res) => setResumo(res.data.resumo))
        .catch(() => setResumo(null));
    }
  }, [open, pendingCaixa]);

  if (!pendingCaixa) return null;

  const saldoEsperado = resumo?.saldoFinal ?? pendingCaixa.initialAmount ?? 0;
  const counted = parseFloat(countedAmount) || 0;
  const diferenca = counted - saldoEsperado;
  const canSubmit = countedAmount !== '' && counted >= 0 && !submitting;
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await caixaService.fechar(pendingCaixa._id, { countedAmount: counted, observacao });
      onClosed();
    } catch (err) {
      toast.error(err.message || 'Falha ao fechar caixa antigo.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title="Caixa expirado — fechamento obrigatório" size="sm">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-amber-500/10 border border-amber-500/40 rounded-md p-3 text-xs text-amber-200 flex gap-2">
          <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" />
          <div>
            O caixa aberto por <strong>{pendingCaixa.openedByName}</strong> em{' '}
            <strong>{fmtDate(pendingCaixa.openedAt)}</strong> ({pendingCaixa.hoursOpen}h atrás)
            precisa ser fechado antes de abrir um novo turno. Confira o dinheiro físico em caixa
            e informe o valor contado abaixo.
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            rows={2}
            maxLength={500}
            placeholder="Ex.: Diferença causada por troco, sangria não registrada, etc."
